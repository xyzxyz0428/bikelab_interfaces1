...
{
  int retry_count = 0;
const int max_retry_count = 10; // Maximum retries before switching to fixed interval
const int base_sleep_time_ms = 100; // Base sleep time in milliseconds
const int max_sleep_time_ms = 5000; // Cap sleep time at 1 minute (60,000 ms)
const int fixed_retry_interval_ms = 5000; // Fixed interval after max retries (5 seconds)
...
while (!streaming_exit_) {
    ...
    if (res != CURLE_OK) {
        if (desired_count_reached_) {
            ...
            char *effectiveUrl;
            curl_easy_getinfo(curlHandle_->handle, CURLINFO_EFFECTIVE_URL, &effectiveUrl);
            RCLCPP_ERROR(this->get_logger(), "Failed to perform streaming request for URL: %s", effectiveUrl);
            ...
            // Handle the error
            RCLCPP_ERROR(this->get_logger(), "Failed to perform streaming request: %s", curl_easy_strerror(res));

            // Exponential backoff with a cap
            int sleep_time_ms;
            if (retry_count < max_retry_count) {
                sleep_time_ms = std::min(base_sleep_time_ms * (1 << retry_count), max_sleep_time_ms); // Exponential increase with cap
                retry_count++;
            } else {
                sleep_time_ms = fixed_retry_interval_ms; // Fixed interval after max retries
            }

            RCLCPP_INFO(this->get_logger(), "Retrying in %d ms...", sleep_time_ms);
